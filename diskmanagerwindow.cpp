...
void diskmanagerWindow::GetCurrentTopology()
{
    ...
    QStringList g=runShellCommand("lsblk -a -l -b -o TYPE,NAME,PTTYPE,FSTYPE,SIZE,PHY-SEC,MODEL,SERIAL,UUID");
    ...
    QStringList fsid=runShellCommand("sh -c \"readlink -f /dev/disk/by-path/* | xargs file -s\"");
    ...
    // GET POSITION OF ALL ARGUMENTS FROM THEFIRST LINE

    int  namepos,pttypepos,fstypepos,sizepos,phy_secpos,modelpos,serialpos,uuidpos;
    {
        QString str=*idx;

        namepos=str.indexOf("NAME");
        pttypepos=str.indexOf("PTTYPE");
        fstypepos=str.indexOf("FSTYPE");
        sizepos=fstypepos+7;
        phy_secpos=str.indexOf("PHY-SEC");
        modelpos=str.indexOf("MODEL");
        serialpos=str.indexOf("SERIAL");
        uuidpos=str.indexOf("UUID");
    }
    ...
    while(idx!=g.constEnd()) {
        ...
        if(!str.startsWith("disk")) { ++idx; continue; }
            dsk.Name=str.mid(namepos,pttypepos-namepos).trimmed();
            dsk.SectorSize=str.mid(phy_secpos,modelpos-phy_secpos).toInt();
            dsk.SectorStart=0;
            dsk.SectorsCount=0;
            dsk.Description=str.mid(modelpos,serialpos-modelpos).trimmed();
            dsk.Description.append("-SN=");
            dsk.Description.append(str.mid(serialpos).trimmed());
            dsk.Level=0;
            dsk.Status=0;
            dsk.InPool.clear();
            dsk.Index=0;
            dsk.Size=str.mid(sizepos,phy_secpos-sizepos).toULongLong();
            dsk.PartType=str.mid(pttypepos,fstypepos-pttypepos).trimmed();
            dsk.FSType=str.mid(fstypepos,7).trimmed();
    ...
    idx=g.constBegin();
    ++idx;  // SKIP FIRST LINE, WE ALREADY HAVE THE POSITION OF THE COLUMNS

    while(idx!=g.constEnd()) {

        QString str=*idx;

        if(!str.startsWith("part")) { ++idx; continue; }
            dsk.Name=str.mid(namepos,pttypepos-namepos).trimmed();
            dsk.SectorSize=str.mid(phy_secpos,modelpos-phy_secpos).toInt();
            dsk.SectorStart=0;
            dsk.SectorsCount=0;
            dsk.Description=str.mid(modelpos,serialpos-modelpos).trimmed();
            dsk.Description.append("-UUID=");
            dsk.Description.append(str.mid(uuidpos).trimmed());
            dsk.Level=1;
            dsk.Status=0;
            dsk.InPool.clear();
            dsk.Index=0;
